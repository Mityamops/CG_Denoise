...
def remove_illumination_gradient_2d(img):
    ...
    # Расширение границ изображения с помощью зеркального отображения
    image = cv2.copyMakeBorder(image, block_y, block_y, block_x, block_x, cv2.BORDER_REFLECT)
    height, width = image.shape

    # Вычисление средних интенсивностей и диапазонов для каждого блока
    avg_intensities = np.zeros((height // block_y, width // block_x))
    ranges = np.zeros((height // block_y, width // block_x))
    ...
    spline_avg = RectBivariateSpline(y, x, avg_intensities)
    spline_range = RectBivariateSpline(y, x, ranges)
    ...
    image = image[block_y:-block_y, block_x:-block_x]
...
def eqhist_lp(image_uint8):
    # Удаление градиента освещения
    image_hist = cv2.equalizeHist(image_uint8)

    processed_image = remove_illumination_gradient_2d(image_hist)
    image_lp = remove_illumination_gradient_lowpass(processed_image)
    ...
    plt.show()
    cv2.imwrite('images/removal_gradient4/eq_hist.png',image_hist)
    cv2.imwrite('images/removal_gradient4/eq_hist+removal_grad.png', processed_image)
    cv2.imwrite('images/removal_gradient4/eq_hist+removal_grad+lp.png', image_lp)


# Пример использования
if __name__ == "__main__":
    # Загрузка изображения
    image_path = 'images/removal_gradient4/W0001F0001T0120Z001C1.tif'
    ...
    #eqhist_lp(image_uint8)

    processed_image=remove_illumination_gradient_2d(image_uint8)

    plt.title('Processed Image')
    plt.imshow(processed_image, cmap='gray')
    plt.show()
    cv2.imwrite('images/removal_gradient4/inverted.png',processed_image)